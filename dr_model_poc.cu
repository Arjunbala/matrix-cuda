...
{
    int m1, n1, k1, m2, n2, k2, m3, n3, k3;
    ...
    int *h_a, *h_b, *h_c, *h_d, *h_e, *h_f, *h_g;
    ...
    cudaMallocHost((void **) &h_f, sizeof(int)*n3*k3);
    cudaMallocHost((void **) &h_g, sizeof(int)*m3*k3);
    ...
    for (int i = 0; i < n2; ++i) {
        for (int j = 0; j < k2; ++j) {
            h_d[i * k2 + j] = rand() % 1024;
        }
    }

    // random initialize matrix F
    for (int i = 0; i < n3; ++i) {
        for (int j = 0; j < k3; ++j) {
            h_f[i * k3 + j] = rand() % 1024;
        }
    }
    ...
    int *d_a, *d_b, *d_c, *d_d, *d_e, *d_f, *d_g;
    ...
    cudaMalloc((void **) &d_f, sizeof(int)*n3*k3);
    cudaMalloc((void **) &d_g, sizeof(int)*m3*k3);

    // copy matrix B,D,F from host to device memory - these are weight matrices
    cudaMemcpy(d_b, h_b, sizeof(int)*n1*k1, cudaMemcpyHostToDevice);
    cudaMemcpy(d_d, h_d, sizeof(int)*n2*k2, cudaMemcpyHostToDevice);
    cudaMemcpy(d_f, h_f, sizeof(int)*n3*k3, cudaMemcpyHostToDevice);
	...
	gpu_matrix_mult<<<dimGrid, dimBlock, 0, 0>>>(d_a, d_b, d_c, m1, n1, k1); // execute on default stream
#else
...
#ifdef USE_CUDA_STREAMS
	gpu_matrix_mult<<<dimGrid2, dimBlock2, 0, 0>>>(d_c, d_d, d_e, m2, n2, k2); // execute on default stream
#else
        gpu_matrix_mult<<<dimGrid2, dimBlock2>>>(d_c, d_d, d_e, m2, n2, k2);
#endif

	// Launch kernel for multiplication 3 - DR model
        grid_rows = (m3 + BLOCK_SIZE - 1) / BLOCK_SIZE;
        grid_cols = (k3 + BLOCK_SIZE - 1) / BLOCK_SIZE;
        dim3 dimGrid3(grid_cols, grid_rows);
        dim3 dimBlock3(BLOCK_SIZE, BLOCK_SIZE);
#ifdef USE_CUDA_STREAMS
	cudaStream_t streams[1];
	cudaStreamCreate(&streams[0]);
	gpu_matrix_mult<<<dimGrid3, dimBlock3, 0, streams[0]>>>(d_c, d_f, d_g, m3, n3, k3); // execute on non-default stream
#else
        gpu_matrix_mult<<<dimGrid3, dimBlock3>>>(d_c, d_f, d_g, m3, n3, k3);
#endif

        // Transfer results from device to host - only DR model result
        cudaMemcpy(h_g, d_g, sizeof(int)*m2*k2, cudaMemcpyDeviceToHost);
